...
class Lindbladian:
    ...
    def exact_spectral_decomposition(self, Lindbladian=None,
                                     eigenvec_left=True):
        ...
        if Lindbladian is None:
            ...
            self.vals, vec_r = linalg.eig(self.L_tot.toarray())
            ...
            dim = Lindbladian.shape[0]
            vals, vec_r_tmp = linalg.eig(Lindbladian.toarray())
            for i in range(dim):
                for j in range(dim):
                    vec_r_tmp[j, i] = round(vec_r_tmp[j, i].real, 14) + \
                        1.j * round(vec_r_tmp[j, i].imag, 14)
            if eigenvec_left:
            ...
            for i in range(dim):
                vec_r.append(vec_r_tmp[:, i].reshape((dim, 1)))
                if eigenvec_left:
                    vec_l.append(vec_l_tmp[:, i].reshape((1, dim)))
            if eigenvec_left:
                return vals, np.array(vec_l), np.array(vec_r)
            return vals, np.array(vec_r)

    def sort_per_N_spin(self, vec, sector=(0, 0)):
        assert np.all(self.Lindbladian.Delta_up * vec == sector[0] * vec)
        assert np.all(self.Lindbladian.Delta_up * vec == sector[1] * vec)
    ...
    def time_evolution_operator(self, time, vals=None,
                                vec_l=None, vec_r=None):
        """Retruns the time propagated Liouvillian vector vec, with vec0 as
        starting vector.
        ...
        """
        if vals is None:
            dim = self.L_tot.shape[0]
            time_evolution_operator = np.zeros((dim, dim), dtype=complex)
            for l1 in np.arange(dim):
                time_evolution_operator += np.exp(self.vals[l1] * time) \
                    * self.vec_r[l1] * self.vec_l[l1]

...
"""
import numpy as np
from define_state_liouville import Liouville_state
from define_state_liouville import c_js_F, c_js_T, cd_js_F, cd_js_T
from define_state_liouville import dot_prod_FL
...
Gamma_mm = Gamma * np.array([[0., 1.], [0., 0.]])
    ...
    for n2 in np.arange(n_orbs):
        ...
        for n3 in np.arange(n_orbs):
            for n4 in np.arange(n_orbs):
                ...
                dp_F = dot_prod_FL(state_out, cdc_F)
                dp_T = dot_prod_FL(state_out, cdc_T)
                ...
                Liouvillian[n_orbs * n2 + n4, n_orbs * n1 + n3] += \
                    - 0.5 * (Gamma_m[n1, n2] + Gamma_m[n3, n4]) * dot_prod_FL(state_out, state_in)
                # Sigma^- \tilde{Sigma^-}
                Liouvillian[n_orbs * n2 + n4, n_orbs * n1 + n3] += \

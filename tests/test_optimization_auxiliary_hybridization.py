...
from src import optimization_auxiliary_hybridization as op_aux
...
def test_const_function_values():
    ...
    renormalized_to_one = op_aux.cost_function(targed_hyb, aux_hyb) == 1.0

    weight = 2 * np.ones(freq.shape)

    weight_times2 = op_aux.cost_function(
        targed_hyb, aux_hyb, weight=weight) == 2.0

    weight = np.array([2.0 * du.heaviside(w, 0) for w in freq])

    weight_heavyside = op_aux.cost_function(
        targed_hyb, aux_hyb, weight=weight) == 1.0
    aux_hyb = fg.FrequencyGreen(
        freq, 1j * np.ones(freq.shape), 1j * np.ones(freq.shape))

    is_equal = op_aux.cost_function(targed_hyb, aux_hyb) == 0.0
...
def test_optimization_ph_symmertry():
    ...
    try:
        ...
        result_nb1 = op_aux.optimization_ph_symmertry(Nb, hybridization,
                                                      x_start=x_start)
        hyb_aux_nb1 = op_aux.get_aux_hyb(result_nb1.x, Nb, freq)
    except ValueError:
        print(f"Minimization for Nb = {Nb}, not converged.")
    hyb_start_nb1 = op_aux.get_aux_hyb(x_start, Nb, freq)
    assert (op_aux.cost_function(hybridization, hyb_aux_nb1)
            < op_aux.cost_function(hybridization, hyb_start_nb1))
...
def test_get_aux_hyb():
    ...
    hyb = op_aux.get_aux_hyb([*es, *ts, *gamma], Nb, freq)
